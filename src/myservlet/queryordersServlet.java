...
public class queryordersServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			while(rs.next()){
				allinf+="订单号:"+rs.getString(1)+";";
				allinf+="订购用户名称:"+rs.getString(2)+";";
				allinf+="订单信息:"+rs.getString(3)+";";
				allinf+="订单总价:"+rs.getString(4);
				allinf+="<br>";
			}
			con.close(); // 关闭连接
		...
		request.getRequestDispatcher("allordersinf.jsp").forward(request, response);;// 重定向到goodsinf.jsp
	}
